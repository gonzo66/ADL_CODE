        ...
        %for iClass=[ 5 18 19 24 25 26 27 34 35 44]
          ...
          % detection_file = [path3 classes{iClass} '/' 'P_' sprintf('%0.2d', iVideo) '.mat'];
           if(exist(detection_file,'file'))
               ...
               thescore2=zeros(size(thescore,1),1);
               %thescore=cat(1,alldata(:).s);
               %theBB=cat(1,alldata(:).xy);
               FN=0;
               detectlengths=zeros(length(gt.fr),1);
               for iGT=1:length(gt.fr)               
                 ...
                 % framepos=min(length(),framepos);
                  ...
                  if(~isempty(framepos))
                      ...
                      nmsbbox=nmsbbox(nmsscore>-0.9,:);
                      nmsscore=nmsscore(nmsscore>-0.9);
                      [top,pick]=esvm_nms([nmsbbox nmsscore], 0.6);
                      ...
                      nmsscore=top(:,end);
                       if(iGT==1)
                              ...
                              st=sum(detectlengths(1:iGT-1));
                       end
                      
                      thescore2(st+1:st+length(nmsscore))=nmsscore;
                      detectlengths(iGT)=size(top,1);
...
%                       end
    ...
    save('RESULTobjectDetection0.6-9.mat','aps');
end
