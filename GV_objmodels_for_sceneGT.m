...
[annot annot_frs] = read_object_annotation(sprintf('%s/object_annot_P_%02d.txt',annotation_dir,iVideo));
        ...
        %for iClass=[ 5 18 19 24 25 26 27 34 35 44]
           ...
           if(exist(detection_file,'file'))
               ...
               FN=0;
               framepos=zeros(length(gt.fr),1);
               for iGT=1:length(gt.fr)               
                  %framepos=find(gt.fr(iGT)==annot_frs);
                  framepostemp=abs(gt.fr(iGT)-frs);
                  [~, frameposa]=min(framepostemp);
                  framepos(iGT)=min(length(boxes),frameposa);
               end
               
               for iFrame=1:length(boxes) %loop over all frames
                   iScene2=P{iVideo}(P{iVideo}(:,1)<=iFrame &  P{iVideo}(:,2)>=iFrame,3);
                       if(isempty(iScene2))
                           iScene2=-1;
                       end
                       iScene=find(scene_labels==iScene2);
                       try
                           nmsbbox=cat(1,boxes{iFrame}.xy);              
                           nmsscore=cat(1,boxes{iFrame}.s);
                           if(sum(framepos==iFrame)>0)
                               [list_overlapped,tmpscores2]=GV_isGTdetected(nmsbbox/2,gt.bbox(iGT,:));                      
                           else
                               tmpscores2=zeros(size(nmsscore));
                           end
...
save('betas_objects_in_scenes_test_ALL.mat','betas','thescore3','target3');
